...
#define INTERVAL_NS (INTERVAL_US * 1000)
    ...
    if (ec_init(ifname)) // TCP/IP의 bind() 같은 거인 듯. 성공 시 true 실패 시 false -> master가 eth에 붙었냐?
    {
        ...
        if (ec_config_init(FALSE) > 0) // int ec_config_init(uint8 usetable); -> 리턴값: 찾은 슬레이브 개수, false는 테이블 기반 off?
        {
            ...
            if (ec_slave[0].state == EC_STATE_OPERATIONAL) // 모든 slaves are operational
            {
                ...
                clock_gettime(CLOCK_MONOTONIC, &target_ts); //절대시간 설정 위한 기준점
                /* cyclic loop */
                for (i = 1; i <= 10000; i++)
                {
                    // 절대 목표 시간 증가
                    target_ts.tv_nsec += INTERVAL_NS;
                    if (target_ts.tv_nsec >= 1000000000)
                    {
                        target_ts.tv_sec++;
                        target_ts.tv_nsec -= 1000000000;
                    }
                    // 절대시간이니까 그냥 interval만큼 늘리면 됨.

                    // 절대 시간까지 sleep
                    clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, &target_ts, NULL); // target만큼 sleep
                    /*
                    #include <time.h>

                    int clock_nanosleep(clockid_t clockid, int flags, const struct timespec *request, struct timespec *remain);

                    nanosleep() 처럼 더 정밀한 멈추기가 가능
                    차이점은 측정하는 클럭을 사용자가 지정할 수 있다는 점(clock_realtime, clock_monotonic 등...)
                    flag가 0이면 지정된 클락의 현재 값에 대한 상대 시간
                    flag가 ABSTIME이면 절대 시간

                    타이머가 만료되어도 wakeup latency가 생긴다. context switch 지연 등으로 예정 시각보다 늦게 깨는 것.
                    usleep()의 경우 늦게 깨도, 그 시간부터 일정 시간만큼 더 sleep.

                    ABSTIME 설정하면 목표 시각이 고정된 타임라인에 설정. 예정 시각보다 늦게 깨도 미리 정해진 절대적인 시각에 깬다

                    */

                    // 깨어난 직후 측정
                    clock_gettime(CLOCK_MONOTONIC, &now_ts);
                    lateness = (now_ts.tv_sec - target_ts.tv_sec) * 1000000000 +
                               (now_ts.tv_nsec - target_ts.tv_nsec);
                    lateness /= 1000; // ns -> µs
                    ...
                    if (lateness >= HIST_MIN)
                    {
                        if (lateness <= HIST_MAX)
                        {
                            hist[lateness]++;
                        }
                        else
                    ...
                    uint64_t converted_now_ts = (uint64_t)now_ts.tv_sec * 1000000000 + (uint64_t)now_ts.tv_nsec;
                    /* 1초마다 누적 히스토그램 출력 */
                    if ((converted_now_ts - last_print_ns) >= 1000000000)
                    {
                        printf("\n=== cycle %d ===\n", i);
                        ...
                        last_print_ns = converted_now_ts;
                    }
                    ...
                    // if (wkc >= expectedWKC)
                    // {
                    //     printf("Processdata cycle %4d, WKC %d, Lateness: %4ld us, O:", i, wkc, lateness);
...
int main(int argc, char *argv[])
{
    ...
    param.sched_priority = 99; // 우선순위 80 (최대 99)

    // SCHED_FIFO: "내가 CPU 잡으면, 내가 놓을 때까지 아무도 못 건드린다"
    if (sched_setscheduler(0, SCHED_FIFO, &param) == -1)
    {
        perror("sched_setscheduler failed");
        return -1; // 실패하면(주로 sudo 안해서) 종료
    }
