   ...
   if (ec_init(ifname)) //TCP/IP의 bind() 같은 거인 듯. 성공 시 true 실패 시 false -> master가 eth에 붙었냐?
   {
       ...
       if ( ec_config_init(FALSE) > 0 )   //int ec_config_init(uint8 usetable); -> 리턴값: 찾은 슬레이브 개수, false는 테이블 기반 off?
      {
         ...
         */
         ...
         // distributed. 즉 모든 슬레이브에 클락을 distribute해서 모두가 동일한 원자 시계를 가질 수 있도록 함.
         // 정확히는 첫 슬레이브의 클락을 기준으로 삼고, 나머지 슬레이브에 대해서는 전파 지연을 계산하여 동기화하는 것
         ...
         */
         ...
         if ((oloop == 0) && (ec_slave[0].Obits > 0)) oloop = 1;  //바이트는 0인데 비트는 있으면, 1바이트 출력
         if (oloop > 8) oloop = 8;  //너무 길면 최대 8바이트만 출력
